...
def global_to_local(drone, lat, lon):
    try:
        position_global = rospy.wait_for_message('mavros%s/global_position/global' % drone, NavSatFix, timeout=0.5)
    except rospy.exceptions.ROSException:
        raise Exception('No global position')

    try:
        pose = rospy.wait_for_message('mavros%s/local_position/pose' % drone, PoseStamped, timeout=0.5)
    except rospy.exceptions.ROSException:
